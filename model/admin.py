...
def checktour(U_id, T_id):
    ...
    cur.execute("select user_1.U_id,tour.T_id,O_id,Name_user,Name_Tour,order_tour.Price,Order_Date,status_Order from order_tour inner join tour on tour.T_id=order_tour.T_id inner join user_1 on user_1.U_id=order_tour.U_id")
    booking = cur.fetchall()
    cur.execute(f"select Name_user,tour.Name_Tour,tour.Vehicle,Order_Date,order_tour.Price,Amount_of_people,user_1.Number_Phone_User,status_Order from order_tour inner join tour on tour.T_id=order_tour.T_id inner join user_1 on user_1.U_id=order_tour.U_id where user_1.U_id='{U_id}' and tour.T_id='{T_id}'")
    ...
    if request.method == 'POST':
        ...
        print(status)
        cur.execute(f"update order_tour set status_Order='{status}' where U_id={U_id} and T_id={T_id}")
        mysql.get_db().commit()
...
def adminuser():
    ...
    if request.method == "POST":
        ...
        if user_id is not None:
            cur.execute(
                f'select U_id,O_id,tour.Name_Tour,time_start,Amount_of_people, order_tour.Price,status_Order from order_tour inner join tour on tour.T_id=order_tour.T_id where U_id={user_id}')
...
def touredit(T_id):
    ...
    if infor_tour is not None:
        ...
        if request.method == 'POST':
            ...
            schedule = request.form['schedule']
            describe = request.form['describe']
            cur.execute(f"Update tour SET P_id={place} ,A_id={str(A_id)}, Name_Tour= '{str(nametour)}', Name_Hotel='{str(namehotel)}', Vehicle='{str(vehicle)}', Time_tour='{str(time_tour)}' ,tour.Price={price},  Schedule_Tour='{str(schedule)}',  Describe_Tour='{str(describe)}',  Image_1={file_names[0]}, Image_2={file_names[1]}, Image_3={file_names[2]}, Image_4={file_names[3]}, Image_5={file_names[4]}  where T_id={T_id} ")
            mysql.get_db().commit()
...
def adminlogin():
    ...
    if request.method == 'POST':
        ...
        cur = mysql.get_db().cursor()
        cur.execute(
            f"select * from admin_1 where Admin_Login='{user_name}' and Password_Admin='{password}'")
        admin = cur.fetchall()
        if 'admin' in session:
